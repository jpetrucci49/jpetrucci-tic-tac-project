...
var boardEvent = __webpack_require__(6);
...
var onPlacePiece = function onPlacePiece(e) {
  if (store.game.over === false && store.played.length < 9) {
    if (store.played.includes(e.target.id)) {
      ...
      api.placePiece(data).then(ui.piecePlaced).catch(ui.pieceFail);
    }
  }
};

var onListGame = function onListGame(e) {
  e.preventDefault();
  api.listGame().then(ui.listGameSuccess).catch(ui.listGameFail);
};
...
var placePiece = function placePiece(data) {
  return $.ajax({
...
var listGame = function listGame() {
  return $.ajax({
    url: config.apiUrl + '/games',
    method: 'GET',
    headers: {
...
var piecePlaced = function piecePlaced(response) {
  ...
  if (store.played.length === 9 && !store.game.over) {
    ...
    var winner = $('div#' + store.played[store.played.length - 1]).text();
    $('#message').text(winner + ' is the Winner!');
  }
};
var pieceFail = function pieceFail() {
  $('#message').text('There was an error.');
};
var listGameSuccess = function listGameSuccess(response) {
  $('#ticTac').hide();
  $('#history').show();
  $('#games').html('');
  response.games.slice(-30).forEach(function (game) {
...
var listGameFail = function listGameFail() {
  $('#message').text('There was an error');
};
module.exports = {
  ...
  pieceFail: pieceFail,
  listGameSuccess: listGameSuccess,
  listGameFail: listGameFail
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var checkWin = function checkWin() {
  var result = winConditions.some(function (condition) {
    //  If line is filled
    if (condition.every(function (id) {
      return store.played.includes(id);
    })) {
      var line = [];
      condition.forEach(function (id) {
        return line.push($('div#' + id).text());
      });
      // line.reduce((curr, next) => curr === next ? next : false)
      // does the same as -~-~-~-
      if (line.every(function (x) {
        return line.every(function (y) {
          return x === y;
        });
      })) {
