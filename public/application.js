...
var fields = __webpack_require__(7);

var onListBook = function onListBook(event) {
  event.preventDefault();
  api.list().then(ui.onListSuccess).catch(ui.onFailure);
};
var onShowBook = function onShowBook(event) {
  event.preventDefault();
  var data = fields(event.target);
  api.show(data).then(ui.onShowSuccess).catch(ui.onFailure);
};
var onDestroyBook = function onDestroyBook(event) {
  event.preventDefault();
  var data = fields(event.target);
  api.destroy(data).then(ui.onDestroySuccess).catch(ui.onFailure);
};
var onCreateBook = function onCreateBook(event) {
  event.preventDefault();
  var data = fields(event.target);
  api.create(data).then(ui.onCreateSuccess).catch(ui.onFailure);
};
var onPatchBook = function onPatchBook(event) {
  event.preventDefault();
  var data = fields(event.target);
  api.patch(data).then(ui.onPatchSuccess).catch(ui.onFailure);
};
...
var onListSuccess = function onListSuccess(response) {
  $('#books').html('');
...
var onPatchSuccess = function onPatchSuccess(response) {
  var book = response.book;
  var bookHTML = '\n    <h4>' + book.title + '</h4>\n    <p>' + book.author + '</p>\n    <p>' + book.id + '</p>\n    <h4>Book Updated</h4>\n  ';
  $('#books').html(bookHTML);
};
